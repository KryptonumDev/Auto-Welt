...
const ScheduleActualExh = ({ dataActual }) => {
    ...
    query actualeExhibit {
        allWpWystawa(
          filter: {wystawa: {informacjeOgolne: {czyWystawaJestAktualnaJezeliNieToJestPlanowana: {eq: true}}}}
        ) {
          edges {
            node {
                slug
    ...
    return (
        <StyledScheduleActualExh>
            ...
            <StyledSliderWrapper>
                <ScheduleSlider scheduleData={data.allWpWystawa.edges} variant="orange" />
            </StyledSliderWrapper>
            <StyledButtonsWrapper>
                <Button
